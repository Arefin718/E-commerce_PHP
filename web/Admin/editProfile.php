        ...
        <div class="registration">
            <form action="editProfile.php" method="post" id="form" enctype="multipart/form-data">
                ...
                <table>
                    ...
                    <tr>
                        <td>Phone</td>
                        <td><input type="text" name="phoneField" value="<?php echo $admin['phone']?>"></td>
       ...
       ($phoneValid==true)) {
        ...
        while ($user=mysqli_fetch_assoc($result)){
          if (isset($_SESSION['adminEmail'])){
              if ($_SESSION['adminEmail']!=$user['email']){
                    if ($email==$user['email']){
                        $emailExists=true;
                    }
              }
        ...
        if (!$emailExists){
            $id=$_SESSION['adminId'];
            $query="UPDATE admin
            SET name='$name', email='$email', address='$address', gender='$gender', phone='$phone'
            WHERE id='$id'";
            $result = mysqli_query($con, $query);
